	...
	<?php
        ...
        {                                  
          ...
          $semester_id = @$_POST['semester_id'];
			...
			$jml=count($nis);
            for ($i=0; $i<$jml; $i++) {
                mysqli_query($connect,"INSERT into tb_nilai(nis,rombel_id,kelas_id,ta_id,semester_id,mapel_id,nilai_tugas) 
                values ('".$nis[$i]."','".$rombel_id."','".$kelas_id."','".$ta_id."','".$semester_id."','".$mapel_id."','".$nilaitugas[$i]."')");
            }
		  ...
		  $kkm = @$_POST['kkm'];
			...
			$hitung_nilai_akhir = ( $n_tugas + $n_pts + $n_pas ) / 3;
				...
				if($status_kelas == 'Naik Kelas'){
					...
					mysqli_query($connect,"UPDATE tb_nilai SET nilai_tugas = '$nilaitugas[$i]', status = '$status_nilai' WHERE nilai_id='$nilai_id[$i]'");
            }
			
			echo "<script>alert('Data Berhasil Diubah')</script>
			<script>window.location='nilai_tugas.php?idmapel=".$_GET['idmapel']."&idrombel=".$_GET['idrombel']."';</script>";
