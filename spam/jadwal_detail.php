        ...
        <div class="card">
          <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap">
              <tbody>
              <?php
              $sql=mysql_query("select b.ta,a.semester,c.kelas from jadwal_kbm a left join tahun_ajaran b on a.id_ta=b.id_ta left join kelas c on a.id_kelas=c.id_kelas where a.id_kelas='$_GET[kls]' AND a.semester='$_GET[sem]' AND a.id_ta='$_GET[thn]' group by b.ta,a.semester,c.kelas");
              $no=1;
              $h=mysql_fetch_array($sql);?>
                <tr>
                  <td class="w-2">Tahun ajaran</td><td class="w-2">:</td> <td><?php echo $h['ta'];?></td>
                </tr>
                <tr>
                  <td>Semester</td><td>:</td><td><span class="text-muted"><?php echo $h['semester'];?></span></td>
                 </tr>
                 <tr> 
                  <td>Kelas</td><td>:</td><td><span class="text-muted"><?php echo $h['kelas'];?></span></td>
                </tr>
              </tbody>
            </table>
            <script>
        ...
        <div class="card">
          ...
          <div class="table-responsive">
            <table class="table card-table table-vcenter text-nowrap datatable">
              ...
              <tbody>
              <?php
              $sql=mysql_query("select a.id_jadwal,a.hari,a.jam_mulai,a.jam_selesai,b.ta,a.semester,c.kelas,d.mapel,e.nama as nama_pemantau,f.nama_tutor from jadwal_kbm a left join tahun_ajaran b on a.id_ta=b.id_ta left join kelas c on a.id_kelas=c.id_kelas left join mapel d on a.id_mapel=d.id_mapel left join user e on a.id_user=e.id_user left join tutor f on a.id_tutor=f.id_tutor where a.id_kelas='$_GET[kls]' AND a.semester='$_GET[sem]' AND a.id_ta='$_GET[thn]' order by a.hari,a.jam_mulai asc");
              $no=1;
              while($h=mysql_fetch_array($sql)){ ?>
                <tr>
                  <td><span class="text-muted"><?php echo $no;?></span></td>
                  <td><?php echo $h['hari'];?></td>
                  <td><?php echo $h['jam_mulai']." - ".$h['jam_selesai'];?></td>
                  <td><?php echo $h['semester'];?></td>
                  <td><?php echo $h['mapel'];?></td>
                  <td><?php echo $h['nama_pemantau'];?></td>
  ...
  <tr>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Tahun ajaran dan Kelas</h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>            
          </div>
    ...
    <td valign="top">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Data jadwal KBM</h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>            
          </div>
          <div class="modal-body">
              <div class="form-group">
                <label>Hari</label>
...
case "delete";
$id=$_GET['id'];
$hapus=mysql_query("DELETE FROM jadwal_kbm WHERE id_jadwal='$id'");
if ($hapus) {
